...
TMD_TOKEN = TMD_TOKEN_ENV or (
    "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImp0aSI6ImU5MTc2NjY1ODE2ODRmODFjNWM4NmEyMGY3YzUzNzZmYTZiZDgxMjQ2ODZmZDQ4YTkwYWE2ZGIwOTcwMjkyNjFiMTViZTliMWY5OWYzNWJkIn0.eyJhdWQiOiIyIiwianRpIjoiZTkxNzY2NjU4MTY4NGY4MWM1Yzg2YTIwZjdjNTM3NmZhNmJkODEyNDY4NmZkNDhhOTBhYTZkYjA5NzAyOTI2MWIxNWJlOWIxZjk5ZjM1YmQiLCJpYXQiOjE3NzgzODYyNDMsIm5iZiI6MTc3ODM4NjI0MywiZXhwIjoxODA5OTIyMjQzLCJzdWIiOiI1Mjg4Iiwic2NvcGVzIjpbXX0.wXTYlG96EkrbhBuk0ssb4V4w5qwbcwQl0M73fSwYXUP1cAJQNumVuG32u_U283Lgfv7RvVczB2VCkudq8YxOckdpDYTtP5GnGHsq9sw_4yETDMykJ5-x8Kwijvz41_DTRcmg4-gkcMaE2u3L0WFRn5FoIUawYod8wzQRyNt0m5pmFk81RSXuv0ecuyJKFfxTN9n6Ypg0Iy5rrKrjvoFWw83kxFIMfOKMTwMrnCa4WzIjK9bAp7FMDkoQNkiPrZraKZ_ZYfFSorcS6Q1GD5uVoEBKvLHEHpeX6q9H0HrmF4d_Az7PtxKHdYThoF7798oaArPPPjlioLYNwlJ2sLrK-8kutxC1UbcOHB8OYZyLjKjjN-LxEUVKJvxk1DPGXAhnP9rzCR1up06yU1jgSiUkqjoUgMRvQkWhMj6D7eJkgYW0mQl1yexjNafL3ZVkGzWGw7djpEh_LMCwG2mZB4SEl5aGp8CVzSetB88ht7_roaYjWi57yJ4kAd1NLX7goxBAgfOvRKuMGtML03QAENVB2dLBfo89t52vZwILBtAZXfeK7MnSwO8rDO37bOOgSZ51q_jtjqS1jf1-XceVz4nI-qV4MEPEwFJyUYzYda6asbQXaQYr720pjsE3L-TvUAgaz_r1QLuJkxbFl0bkoq9t_G5hfv8xO-phSTFlcNU-7AY"
)
...
# Met.no returns 403 if User-Agent looks like a placeholder (e.g. contains "example.com").
# Open-Meteo also benefits from an identifiable UA. Override with WEATHER_HTTP_USER_AGENT if needed.
def _effective_weather_user_agent() -> str:
    raw = (os.getenv("WEATHER_HTTP_USER_AGENT") or "").strip()
    if not raw:
        return "SeniorOntologyRouteScorer/1.0"
    banned = ("example.com", "example.org", "test.com", "localhost")
    low = raw.lower()
    if any(b in low for b in banned):
        print(
            "[Humidity] WEATHER_HTTP_USER_AGENT looks like a placeholder; "
            "using default (set a real app name + URL per Met.no terms)."
        )
        return "SeniorOntologyRouteScorer/1.0"
    return raw


WEATHER_HTTP_USER_AGENT = _effective_weather_user_agent()
...
HUMIDITY_CACHE_TTL = 1800  # 30 minutes (successful RH)
HUMIDITY_FAILURE_CACHE_TTL = 900  # 15 minutes — avoid hammering APIs when all sources fail
OPEN_METEO_BACKOFF_UNTIL = 0.0


def _open_meteo_backoff_seconds(status_code: int, body_text: str) -> float:
    """After 429, back off longer when Open-Meteo daily quota is exhausted."""
    if status_code != 429:
        return 600.0
    t = (body_text or "").lower()
    if "daily" in t and "limit" in t:
        # Quota resets next UTC day; add a small buffer.
        now_utc = datetime.now(timezone.utc)
        next_day = (now_utc + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0)
        return max(3600.0, (next_day - now_utc).total_seconds() + 120.0)
    return 600.0
...
async def fetch_humidity(client, lat, lon):
    ...
    try:
        ...
        if h_key in HUMIDITY_CACHE:
            ...
            age = h_now - ts
            if rh_cached is not None and age < HUMIDITY_CACHE_TTL:
                print(f"[Humidity] cache hit lat={lat} lon={lon} rh={rh_cached}")
                return float(rh_cached)
            if rh_cached is None and age < HUMIDITY_FAILURE_CACHE_TTL:
                print(f"[Humidity] cache hit (recent miss) lat={lat} lon={lon}; skipping providers")
                return None
        ...
        tmd_resp = await client.get(
            ...
            headers={
                ...
                "user-agent": WEATHER_HTTP_USER_AGENT,
        ...
        try:
            ...
            if rf.status_code != 200:
                ...
                if rf.status_code == 429:
                    backoff = _open_meteo_backoff_seconds(429, rf.text or "")
                    OPEN_METEO_BACKOFF_UNTIL = time.time() + backoff
                    print(f"[Humidity] open-meteo 429 backoff {int(backoff)}s")
        ...
        try:
            ...
            if rm.status_code != 200:
                ...
                if rm.status_code == 403:
                    print(
                        "[Humidity] met.no 403: set WEATHER_HTTP_USER_AGENT to a unique id + contact URL "
                        "(must not use example.com); see api.met.no Locationforecast docs (Usage / User-Agent)."
                    )
                raise RuntimeError(f"met.no status={rm.status_code}")

            jm = rm.json()
            ts = ((jm.get("properties") or {}).get("timeseries") or [])
            if not ts:
                print(f"[Humidity] fallback2 missing timeseries for lat={lat} lon={lon}")
                raise RuntimeError("met.no missing timeseries")
            ...
            if rh_val is None:
                ...
                raise RuntimeError("met.no missing relative_humidity")
        ...
        try:
            ...
            if rw.status_code != 200:
                body_snip = (rw.text or "")[:300].replace("\n", " ")
                print(f"[Humidity] fallback3 status={rw.status_code} lat={lat} lon={lon}; body_snip='{body_snip}'")
                raise RuntimeError(f"wttr.in status={rw.status_code}")
            jw = rw.json()
            cur = (jw.get("current_condition") or [{}])[0]
            hum = cur.get("humidity")
            if hum is None:
                print(f"[Humidity] fallback3 missing humidity for lat={lat} lon={lon}")
                raise RuntimeError("wttr.in missing humidity")
            rh_val = float(hum)
            print(f"[Humidity] fallback3 source=wttr.in lat={lat} lon={lon} rh={rh_val}")
            HUMIDITY_CACHE[h_key] = (h_now, rh_val)
            return rh_val
        except Exception as fe3:
            print(f"[Humidity] fallback3 request failed for lat={lat} lon={lon}: {fe3}")

        HUMIDITY_CACHE[h_key] = (h_now, None)
        return None
    except Exception as e:
